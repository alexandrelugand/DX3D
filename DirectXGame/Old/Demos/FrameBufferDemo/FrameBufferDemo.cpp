...
#include <App/FrameBufferDemo/FrameBufferDemo.h>
...
namespace Ge
{
	...
	void FrameBufferDemo::OnCreate()
	{
		...
		m_monitor_mesh = GraphicsEngine::Get()->GetMeshManager()->CreateMesh(L"monitor");
		m_monitor_mat = GraphicsEngine::Get()->CreateMaterial(L"DirectionalLight");
		m_monitor_mat->AddTexture(m_sphere_tex);
		m_screen_mat = GraphicsEngine::Get()->CreateMaterial(L"DirectionalLight");
		...
		m_mini_game.SetWindowSize(rect);
