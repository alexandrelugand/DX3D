...
namespace Ge
{
	...
	void MiniGame::UpdateModel(const Vector3D& position, const Vector3D& rotation, const Vector3D& scale, const std::vector<MaterialPtr>& materials)
	{
		Constant ct;
	...
	void MiniGame::DrawMesh(const MeshPtr& mesh, const std::vector<MaterialPtr>& materials)
	{
	...
	{
		const int width = m_window_size.Width();
		const int height = m_window_size.Height();

		m_delta_mouse_x = mouse_pos.m_x - (m_window_size.m_left + width / 2.0f);
		m_delta_mouse_y = mouse_pos.m_y - (m_window_size.m_top + height / 2.0f);
	}
