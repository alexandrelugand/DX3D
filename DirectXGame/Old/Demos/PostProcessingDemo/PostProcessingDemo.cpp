...
#include <App/PostProcessingDemo/PostProcessingDemo.h>
...
namespace Ge
{
	void PostProcessingDemo::OnCreate()
	{
		...
		mp_render_target = GraphicsEngine::Get()->GetTextureManager()->CreateTexture(rect, Texture::Type::RenderTarget);
		mp_depth_stencil = GraphicsEngine::Get()->GetTextureManager()->CreateTexture(rect, Texture::Type::DepthStencil);
		...
		MaterialSlot quad_material_slot[] = {
			{0, 6, 0}
		};

		mp_quad_mesh = GraphicsEngine::Get()->GetMeshManager()->CreateMesh(
			quad_vertex_list, 4,
			quad_index_list, 6,
			quad_material_slot, 1
		);

		mp_post_process_material = GraphicsEngine::Get()->CreateMaterial(L"PostProcessEffects\\Distortion");
		mp_post_process_material->AddTexture(mp_render_target);
	...
	void PostProcessingDemo::Render()
	{
		...
		GraphicsEngine::Get()->GetRenderSystem()->GetImmediateDeviceContext()->ClearRenderTargetColor(mp_render_target, 0, 0.3f, 0.4f, 1);
		GraphicsEngine::Get()->GetRenderSystem()->GetImmediateDeviceContext()->ClearDepthStencil(mp_depth_stencil);
		GraphicsEngine::Get()->GetRenderSystem()->GetImmediateDeviceContext()->SetRenderTarget(mp_render_target, mp_depth_stencil);

		//Set viewport of render target in which we have to draw"((
		//const auto rect = GetClientWindowRect();
		const Rect viewport_size = mp_render_target->GetSize();
		...
		//Render quad mesh
		DistortionEffectData distortion_effect_data;
		distortion_effect_data.distortion_level = m_distortion_level;

		m_materials.clear();
		m_materials.push_back(mp_post_process_material);
		mp_post_process_material->SetData(&distortion_effect_data, sizeof(DistortionEffectData));
		DrawMesh(mp_quad_mesh, m_materials);
	...
	void PostProcessingDemo::UpdateModel(const Vector3D& position, const Vector3D& rotation, const Vector3D& scale, const std::vector<MaterialPtr>& materials)
	{
		Constant ct;
	...
	void PostProcessingDemo::UpdateSpaceShip()
	{
		...
		if (m_turbo_mode)
		{
			m_spaceship_speed = 505.0f;
			if (m_forward != 0.0f)
				m_distortion_level = Lerp(m_distortion_level, 0.6f, m_delta_time * 0.4f);
		}
		else
			m_distortion_level = Lerp(m_distortion_level, 1.0f, m_delta_time * 0.4f);
	...
	void PostProcessingDemo::DrawMesh(const MeshPtr& mesh, const std::vector<MaterialPtr>& materials)
	{
	...
	{
		const auto rect = GetClientWindowRect();
		mp_swap_chain->Resize(rect.Width(), rect.Height());

		mp_render_target = GraphicsEngine::Get()->GetTextureManager()->CreateTexture(rect, Texture::Type::RenderTarget);
		mp_depth_stencil = GraphicsEngine::Get()->GetTextureManager()->CreateTexture(rect, Texture::Type::DepthStencil);

		mp_post_process_material->RemoveTexture(0);
		mp_post_process_material->AddTexture(mp_render_target);
